...
before(async () => {
  ...
  await prisma.connectedAccount.create({ data: { userId, provider: "microsoft", accountLabel: "test@example.com", status: "active", scopes: encodeJson(["Mail.Read", "Mail.ReadWrite", "Mail.Send", "Calendars.Read", "Calendars.ReadWrite", "Files.Read"]), encryptedAccessToken: encryptConnectorToken("graph-token"), expiresAt: new Date(Date.now() + 3_600_000) } });
});
