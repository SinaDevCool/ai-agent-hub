...
async function googleRequest(url: string, init: RequestInit, timeoutMs = env.HOUSEHOLD_PROVIDER_TIMEOUT_MS) {
  if (!env.GOOGLE_PLACES_API_KEY) throw httpError(503, "Google Places is not configured.", "google_places_not_configured"); const controller = new globalThis.AbortController(); const timer = setTimeout(() => controller.abort(), timeoutMs);
...
export async function searchGooglePlacesProviders(input: { query: string; timeoutMs?: number }) {
  const body = await googleRequest("https://places.googleapis.com/v1/places:searchText", { method: "POST", headers: { "Content-Type": "application/json", "X-Goog-FieldMask": fields }, body: JSON.stringify({ textQuery: input.query, pageSize: 10 }) }, input.timeoutMs);
...
export async function searchLiveHouseholdProviders(input: { serviceType: string; location: string }) { requireLive(); return searchGooglePlacesProviders({ query: `${input.serviceType} in ${input.location}` }); }

export async function getGooglePlaceDestination(placeId: string, timeoutMs?: number) {
  if (!/^[A-Za-z0-9_-]{10,300}$/.test(placeId)) throw httpError(400, "A valid Google Place ID is required.", "invalid_place_id");
  const body = await googleRequest(`https://places.googleapis.com/v1/places/${encodeURIComponent(placeId)}`, { method: "GET", headers: { "X-Goog-FieldMask": "id,businessStatus,websiteUri" } }, timeoutMs);
...
export async function getHouseholdProviderDestination(placeId: string) { requireLive(); return getGooglePlaceDestination(placeId); }
