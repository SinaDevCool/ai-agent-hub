...
export async function searchMicrosoftDriveFiles(input: { userId: string; query: string; limit?: number }) {
  const response = await microsoftFetch<{ value?: Array<{ id: string; name: string; webUrl?: string; lastModifiedDateTime?: string; file?: { mimeType?: string } }> }>({ userId: input.userId, path: `/me/drive/root/search(q='${encodeURIComponent(input.query.replace(/'/g, "''"))}')?$top=${Math.min(Math.max(input.limit ?? 10, 1), 25)}`, scopes: ["Files.Read"] });
