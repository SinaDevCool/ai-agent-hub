    ...
    <div className={props.className} id="settings">
      ...
      <section className={`settings-connector-card settings-view settings-view-advanced ${activeView === "advanced" ? "is-active" : ""}`}>
        ...
        {props.lifePlatform.financeSummary ? <>
          ...
          <div className="workflow-list" aria-label="Recent financial transactions" role="list">{props.lifePlatform.financeSummary.transactions.slice(0, 6).map((transaction) => <div className="workflow-row" key={transaction.id} role="listitem"><div><strong>{transaction.merchantName ?? transaction.name}</strong><span>{new Intl.DateTimeFormat(undefined, { dateStyle: "medium" }).format(new Date(transaction.date))} · {transaction.categoryPrimary?.replace(/_/g, " ") ?? "Uncategorized"}</span></div><span>{formatCurrency(transaction.amount, transaction.currency)}</span></div>)}</div>
