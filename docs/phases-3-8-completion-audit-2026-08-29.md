...
| 4 — Live travel | Duffel and Amadeus flight search, Amadeus Hotel Search v3 inventory and selected-offer refresh, normalized live offers, bounded search policy, approval lifecycle, and hosted-checkout foundation | A hotel hosted-checkout partner, provider agreements, support, refunds, regional terms, and real staging purchases remain external gates | Engineering complete behind flags; do not call the phase production-complete |
...
The next autonomous engineering slice should be the consolidated release-readiness dashboard and evidence view. Production activation work should begin with real staging credentials, a hotel hosted-checkout partner, and the acceptance drills above when the user is available to sign in.
