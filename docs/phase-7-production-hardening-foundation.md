...
- Requests enqueue sanitized durable jobs containing only the request identifier. The export executor has its own disabled flag and writes encrypted, short-lived local artifacts. No deletion executor is registered, so enabling the API alone cannot delete user data.
- Artifact references are designed to store opaque storage identifiers, never public download URLs or export contents.

## Remaining production gates

Phase 7 is not complete until dated evidence exists for security review, dependency and secret scanning, backup restore, manual WCAG 2.2 AA review, representative load tests, privacy/legal approval, incident exercises, monitoring alerts, rollback, and production smoke tests. Enabling export processing additionally requires an authenticated download flow, artifact cleanup worker, retention validation, identity re-verification, and approved storage. Deletion still requires a separately reviewed executor.
