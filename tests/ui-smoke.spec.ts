...
test("loads dashboard and exercises safe primary UI flows", async ({ page }) => {
  ...
  await nav.getByRole("link", { name: "My Agents", exact: true }).click();
  await expect(page.locator("#helpers")).toContainText("No agents yet");
  await nav.getByRole("link", { name: "Private Data", exact: true }).click();
  ...
  await expect(nav.getByRole("link", { name: "Discover", exact: true })).toHaveClass(/nav-active/);
  ...
  await nav.getByRole("link", { name: "Private Data", exact: true }).click();
  await expect(nav.getByRole("link", { name: "Private Data", exact: true })).toHaveClass(/nav-active/);
  await expect(page.locator("#vault")).toContainText("Saved info");
  ...
  await expect(nav.getByRole("link", { name: "Creator Studio", exact: true })).toHaveCount(0);
  ...
  await expect(page.locator("#clearance")).toContainText("Private Data Access");
  ...
  await nav.getByRole("link", { name: "My Agents", exact: true }).click();
  await expect(nav.getByRole("link", { name: "My Agents", exact: true })).toHaveClass(/nav-active/);
  await expect(page.locator("#helpers").getByText("Your agents", { exact: true })).toBeVisible();
  await expect(page.getByLabel("Search agents")).toBeVisible();
  await nav.getByRole("link", { name: "Discover", exact: true }).click();
  await expect(nav.getByRole("link", { name: "Discover", exact: true })).toHaveClass(/nav-active/);
  await expect(page.locator(".marketplace-panel").getByLabel("Find agents", { exact: true })).toBeVisible();
  ...
  if (await addHelperButton.count()) {
    ...
    const installDialog = page.getByRole("dialog", { name: /Set up Budget Guard/ });
    await installDialog.getByRole("button", { name: "Continue" }).click();
    await installDialog.getByRole("button", { name: "Continue" }).click();
    const installResponse = page.waitForResponse((response) => response.request().method() === "POST" && /\/api\/marketplace\/agents\/[^/]+\/install$/.test(response.url()));
    await installDialog.getByRole("button", { name: "Add Agent" }).click();
    await expect((await installResponse).ok()).toBeTruthy();
  }
  await expect(nav.getByRole("link", { name: "My Agents", exact: true })).toHaveClass(/nav-active/);
  ...
  await expect(page.locator(".audit-panel")).toContainText("Private info access allowed");
  ...
  await page.getByPlaceholder("Ask what you want help with…").fill("Transfer funds");
  await page.getByRole("button", { name: "Send" }).click();
  ...
  await nav.getByRole("link", { name: "Activity", exact: true }).click();
  await expect(nav.getByRole("link", { name: "Activity", exact: true })).toHaveClass(/nav-active/);
  await expect(page.locator("#activity").getByText("Activity & Receipts", { exact: true })).toBeVisible();
  await expect(page.locator(".audit-panel")).toContainText(/Saved info changed|Vault Write/);
  ...
  await page.locator(".agent-profile-tabs").getByRole("tab", { name: "Settings" }).click();
  await page.locator(".agent-tab-panel").getByRole("button", { name: "Remove Budget Guard" }).click();
...
test("mobile layout keeps the app simple and tab-focused", async ({ page }) => {
  ...
  const nav = page.locator(".nav-rail");
  await expect(nav.getByRole("link", { name: "Discover", exact: true })).toHaveAttribute("data-mobile-label", "Discover");
  await expect(nav.getByRole("link", { name: "My Agents", exact: true })).toHaveAttribute("data-mobile-label", "Agents");
  await expect(nav.locator(".nav-item-group > a:visible")).toHaveCount(5);
  ...
  await expect(page.locator("#helpers")).toContainText("Your agents");
  ...
  await page.locator(".topbar").getByRole("link", { name: "Settings", exact: true }).click();
  ...
  const dailyTaskCard = page.locator(".marketplace-card").filter({ hasText: "Daily Task Agent" }).first();
  await expect(dailyTaskCard.getByRole("button", { name: "Add Agent" })).toBeHidden();
  await dailyTaskCard.getByRole("button", { name: /View details/ }).click();
  const mobileSheet = page.locator(".marketplace-detail-sheet");
  await expect(mobileSheet).toContainText("Daily Task Agent");
  ...
  const mobileInstallDialog = page.getByRole("dialog", { name: "Set up Daily Task Agent" });
  await mobileInstallDialog.getByRole("button", { name: "Continue" }).click();
  await mobileInstallDialog.getByRole("button", { name: "Continue" }).click();
  const mobileInstallResponse = page.waitForResponse((response) => response.request().method() === "POST" && /\/api\/marketplace\/agents\/[^/]+\/install$/.test(response.url()));
  await mobileInstallDialog.getByRole("button", { name: "Add Agent" }).click();
  await expect((await mobileInstallResponse).ok()).toBeTruthy();
  await expect(nav.getByRole("link", { name: "My Agents", exact: true })).toHaveClass(/nav-active/);
  ...
  await expect(page.locator(".agent-chat-panel")).toContainText("Ask Daily Task Agent");
  ...
  await page.getByPlaceholder("Ask what you want help with…").fill("Share personal info");
  await page.getByRole("button", { name: "Send" }).click();
  ...
  await nav.getByRole("link", { name: "Activity", exact: true }).click();
  await expect(page.locator(".audit-panel")).toBeVisible();
  await expect(page.locator(".audit-panel")).toContainText("Activity & Receipts");
  await expect(page.locator(".audit-panel")).toContainText("Waiting");
  await expect(page.locator(".audit-panel")).toContainText(/paused before|waited for you|is waiting for you|Nothing continued/);
  await expect(page.locator(".audit-panel")).toContainText(/You denied this action|will not continue|Denied\. Nothing continued\.|Nothing continues unless you allow it\./);
  ...
  await expect(page.locator(".agent-list")).toBeVisible();
  await expect(page.locator(".detail-panel")).toBeHidden();
  await page.locator(".mobile-agent-card").filter({ hasText: "Daily Task Agent" }).getByRole("button", { name: "Chat" }).click();
  await expect(page.locator(".detail-panel")).toBeVisible();
